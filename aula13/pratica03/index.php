<?php 
  ...
  $pessoasSerializadas = serialize($pessoas);
  
  // Usar os conversores definidos nos models: toArray() / toJson()
  // Converte cada objeto Pessoa em array chamando $p->toArray()
  $all = array_map(function($p){
    if (is_object($p) && method_exists($p, 'toArray')) return $p->toArray();
    return is_object($p) ? (array)$p : $p;
  }, $pessoas);

  $arquivoJson = __DIR__ . DIRECTORY_SEPARATOR . 'pessoas.json';
  file_put_contents($arquivoJson, json_encode($all, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));

  echo "<p>Arquivo gerado: <strong>" . basename($arquivoJson) . "</strong></p>";
?>